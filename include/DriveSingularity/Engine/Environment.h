...
public: // API
  Environment(const roadmap::RoadMap &roadMap, const roadmap::Graph &roadGraph)
    ...
    for (const auto &roadpair : roadMap.getRoads()) {
      ...
      if (road->getRoadType() == roadmap::RoadType::Straight) {
        auto straightRoad =
            std::dynamic_pointer_cast<roadmap::StraightRoad>(road);
        if (flag) {
        ...
        for (size_t k = 0; k < straightRoad->getLanes().size(); ++k) {
          ...
          if (position.getX() < videoAnchor1.getX() ||
              position.getY() < videoAnchor1.getY() ||
              position.getX() > videoAnchor2.getX() ||
              position.getY() > videoAnchor2.getY()) {
    ...
    agentActionType = control::ACTION_SPACE_TYPE::DISCRETE; // default action
                                                            // space is discrete
  }

  void setLogger(std::string &fpath) {
    logger.setFilePath(fpath);
    logger.clear();
  }
  void record() {
    logger.log(epoch, vehicles);
    logger.log(roadMap->getNodes());
  }
  ...
  void initGenerator(size_t socialVehicleNum,
                     std::pair<double, double> halfLengthRange,
                     std::pair<double, double> halfWidthRange,
                     std::pair<double, double> velocityRange,
                     std::pair<double, double> targetVelocityRange) {
  ...
  int loadGenerators(const std::list<std::unordered_map<std::string, float>>
                         &generator_config);
...
public: // inner callable
  ...
  VehicleGenerator &getVehicleGenerator() { return vehicleGenerator; }
...
Observation
drawObservation(const VectorD &center, std::size_t rangeLength,
                std::size_t rangeWidth,
